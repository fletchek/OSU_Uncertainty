...
RT_cond$ID.num <- match(RT_cond$ID, unique(RT_cond$ID))
...
RT_cond$condition.code <- match(RT_cond$condition, levels(RT_cond$condition))

### same thing for block
# dat$block <- as.factor(dat$block)
# dat$block <- factor(dat$block, levels(dat$block)[c(2, 1)]) # put in order unaided, aide
RT_cond$aided.code <- match(RT_cond$aided, levels(RT_cond$aided))
...
dataList <- list(
  k = length(RT_cond$ID),
  n = length(unique(RT_cond$ID)),
  ID = RT_cond$ID.num,
  form = RT_cond$condition.code,
  block = RT_cond$aided.code,
  d = RT_cond$RT_mean, c = RT_cond$RT_mean,
  d_M = mean(RT_cond$RT_mean), d_SD = sd(RT_cond$RT_mean),
  c_M = mean(RT_cond$RT_mean), c_SD = sd(RT_cond$RT_mean)#,
)
