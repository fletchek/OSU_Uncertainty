...
RT_signal_FH$ID.num <- match(RT_signal_FH$ID, unique(RT_signal_FH$ID))
...
RT_signal_FH$condition.code <- match(RT_signal_FH$condition, levels(RT_signal_FH$condition))

### same thing for block
# dat$block <- as.factor(dat$block)
# dat$block <- factor(dat$block, levels(dat$block)[c(2, 1)]) # put in order unaided, aide
RT_signal_FH$aided.code <- match(RT_signal_FH$aided, levels(RT_signal_FH$aided))
...
dataList <- list(
  k = length(RT_signal_FH$ID),
  n = length(unique(RT_signal_FH$ID)),
  ID = RT_signal_FH$ID.num,
  form = RT_signal_FH$condition.code,
  block = RT_signal_FH$aided.code,
  d = RT_signal_FH$RT_F, c = RT_signal_FH$RT_H,
  d_M = mean(RT_signal_FH$RT_F), d_SD = sd(RT_signal_FH$RT_F),
  c_M = mean(RT_signal_FH$RT_H), c_SD = sd(RT_signal_FH$RT_H)#,
)
